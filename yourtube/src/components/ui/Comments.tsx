        ...
        <div key={comment._id} className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            {editingCommentId === comment._id ? (
              ...
              <p>{translated[comment._id] || comment.commentbody}</p>
            )}

            <small className="flex items-center text-muted-foreground mt-1">
              <MapPin className="w-3 h-3 mr-1" /> {comment.city}
            </small>

            <div className="flex gap-4 mt-2 text-sm items-center text-muted-foreground">
              <button onClick={() => handleReact(comment._id, "like")} className={`flex items-center gap-1 hover:text-black ${comment.likedBy?.includes(user?._id) ? 'text-black' : ''}`}>
                <ThumbsUp className={`w-4 h-4 ${comment.likedBy?.includes(user?._id) ? 'fill-black' : ''}`} /> {comment.likes}
              </button>

              <button onClick={() => handleReact(comment._id, "dislike")} className={`flex items-center gap-1 hover:text-black ${comment.dislikedBy?.includes(user?._id) ? 'text-black' : ''}`}>
                <ThumbsDown className={`w-4 h-4 ${comment.dislikedBy?.includes(user?._id) ? 'fill-black' : ''}`} /> {comment.dislikes}
              </button>
              ...
              {comment.userid === user?._id && (
                <>
                  <button onClick={() => handleEdit(comment)} className="flex items-center gap-1 hover:text-black">
